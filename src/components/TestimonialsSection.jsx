...
const testimonials = [
  {
    quote: "More than just a vendor—Sports Wizards felt like partners on our campus project.",
  ...
  {
    quote: "The quality of their work exceeded all our expectations. Highly recommended!",
    name: "Mr. Arjun Mehta",
...
const TestimonialCard = ({ quote, name, title }) => (
  <div className="text-center p-4 h-full flex flex-col justify-center items-center w-full">
    <p className="text-bg-foreground-secondary italic text-lg md:text-xl mb-6">"{quote}"</p>
    <div>
...
const TestimonialsSection = () => {
  ...
  const updateBorders = useCallback((swiper) => {
    ...
    swiper.slides.forEach(slide => {
      slide.querySelector('.gradient-border-r')?.classList.remove('hide-border');
    });
    ...
    if (lastSlide) {
      lastSlide.querySelector('.gradient-border-r')?.classList.add('hide-border');
    }
  }, []);
